...
let ssi           = require('browsersync-ssi');
...
let path = {
	build: {
		...
		favicon: project_name + "/img/favicon",
		fonts: project_name + "/fonts/",
		json: project_name + "/json/",
		svg: project_name + "/img/",
	},
	src: {
...
let buildDev = gulp.series(clean, gulp.parallel(fontsBuild, copyFolders, json, html, css, js, favicon, images, svgSprite));
let watch = gulp.series(buildDev, gulp.parallel(watchFiles, browserSync));
let build = gulp.parallel(htmlBuild, cssBuild, svgSprite, jsBuild, imagesBuild);
